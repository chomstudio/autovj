...
現在はUI/UX改善と信頼度低下中のグリッチ演出を追加した **M0.6** です。最終的な企画は `auto_vj_project_spec.md`、現在の実装仕様は `prototype_spec.md`、ユーザーからの次の指示は `prompt.md` を参照してください。
...
- 1つのボタンで音声入力を一時停止・再開
- メイン画面と詳細設定オーバーレイを分離
- 詳細設定から再生・クロスフェード・ブレンド・グリッチしきい値を保存して即時反映
- 登録動画一覧を詳細設定内の専用スクロール領域へ表示
...
- BPMは自動推定値です。DBには解析元を保存できますが、手動修正UIは未実装です。
