...
更新日: 2026-07-17
現在の段階: M0.7 画面分離・WASAPIループバック・BPM安全補正まで実装完了
...
現在の主な未実装項目は、テンポとピッチを同時変更する入力への対応、専用Windows映像出力、LAN公開とPIN認証、配布用インストーラーである。ミックス時の後入り曲判定とBPM手動修正UIは、現在の運用方針では実装対象外とする。
