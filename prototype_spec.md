...
更新日: 2026-07-16
現在の段階: M0.6 UI/UX改善・信頼度低下演出まで実装完了
...
Web UIは既定で `http://127.0.0.1:5180/` に公開し、通常起動では既定ブラウザを自動表示する。
...
- 映像領域はUIの高さやスクロール量に影響されず、画面内の固定領域を維持する。
- 画面全体にはスクロールバーを出さず、右側UIパネルだけを必要に応じてスクロールする。
...
- ページ遷移せずメイン画面の上へ表示し、表示中も音声入力と曲検出を継続する。
- 登録動画一覧は専用領域内で縦スクロールする。
- 未検出待機時間、再同期許容差、クロスフェード有効化、切り替え時間、ブレンド有効化、ランダム選択、ブレンドモード、グリッチしきい値を編集できる。
- ブレンドモードはチェックボックスで複数選択する。
- 「設定を保存」は対象値を `config.yaml` へ保存し、実行中の設定へ即時反映して閉じる。
- 「キャンセル」は変更を保存せず閉じる。
...
現在の主な未実装項目は、WASAPIループバック入力、テンポとピッチを同時変更する入力への対応、ミックス時の後入り曲判定、BPM手動修正UI、専用Windows映像出力、LAN公開とPIN認証、配布用インストーラーである。
