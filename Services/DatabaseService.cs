...
public sealed class DatabaseService(AppConfig config)
{
    ...
    public async Task UpsertTrackAsync(TrackRecord track, CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText = """
            INSERT INTO tracks (
                ...
                file_size, file_modified_utc, updated_at)
            VALUES (
                $name, $video, $duration, $fingerprint,
                $fileSize, $fileModifiedUtc, $updated)
    ...
    public async Task<List<CatalogEntry>> GetCatalogEntriesAsync(CancellationToken cancellationToken = default)
    {
        ...
        await using var connection = await OpenConnectionAsync(cancellationToken);
        var command = connection.CreateCommand();
        command.CommandText = "SELECT id, video_path, file_size, file_modified_utc FROM tracks ORDER BY id;";
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
        while (await reader.ReadAsync(cancellationToken))
        {
            entries.Add(new CatalogEntry(reader.GetInt64(0), reader.GetString(1), reader.GetInt64(2), reader.GetString(3)));
        }
        return entries;
    }
    ...
    public async Task<List<TrackSummary>> GetTrackSummariesAsync(CancellationToken cancellationToken = default)
    {
        ...
        await using var connection = await OpenConnectionAsync(cancellationToken);
        var command = connection.CreateCommand();
        command.CommandText = "SELECT id, name, video_path, duration_seconds FROM tracks ORDER BY id;";
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
        while (await reader.ReadAsync(cancellationToken))
        {
            tracks.Add(new TrackSummary(reader.GetInt64(0), reader.GetString(1), Path.GetFileName(reader.GetString(2)), reader.GetDouble(3)));
        }
        return tracks;
    }
